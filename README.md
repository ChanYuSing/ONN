...
├── calibration/
│   ├── calibrate_lcd_hardware.py   # Interactive CLI: measure fill factor, contrast ratio
│   └── calibrate_multilayer.py     # Interactive CLI: measure layer spacing, alignment errors
├── matlab/
│   ├── onn_simulate.m              # MATLAB independent validation script
│   └── export/                     # .mat files generated by export_matlab.py (gitignored)
├── report/                    # Technical report (Markdown sources + figures)
├── paper/                     # IEEE-style technical paper (Markdown sources)
├── docs/                      # Submission documents (final report, poster, technical paper PDFs)
├── requirements.txt
└── .gitignore
```
...
### 6. (Optional) Calibrate real hardware

```bash
python calibration/calibrate_lcd_hardware.py    # single LCD parameters
python calibration/calibrate_multilayer.py      # inter-layer spacing & alignment
```
